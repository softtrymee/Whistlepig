...
English source strings live in `ElementX/Resources/Localizations/en.lproj`. When adding a
new string, update the English source and run SwiftGen through the normal project
generation workflow. Other translations are maintained as checked-in resources in this
private fork; no external translation service is required for local development.
...
The changelog is maintained in `CHANGES.md`. Add a concise entry when a user-visible
change is made; release notes are prepared manually for this private fork.
...
Run SwiftLint and SwiftFormat locally before committing. The test command also produces
local xcresult and coverage artifacts when requested; no hosted PR service is required.
