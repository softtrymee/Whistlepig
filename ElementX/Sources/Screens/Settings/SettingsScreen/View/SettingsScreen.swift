...
struct SettingsScreen: View {
                ...
                Button {
                    ...
                    HStack(spacing: 12) {
                        LoadableAvatarImage(url: context.viewState.userProfile.avatarURL,
                            ...
                            .padding(2)
                            .background(WhistlepigBrand.marmotMist, in: Circle())
                            .overlay(Circle().stroke(WhistlepigBrand.marmotBrown.opacity(0.42), lineWidth: 1))
    ...
    private var generalSection: some View {
        Section {
            ...
            ListRow<EmptyView, EmptyView, WhistlepigAccentPickerRow, WhistlepigAccent>(kind: .custom {
                WhistlepigAccentPickerRow(selection: Binding(get: { context.whistlepigAccent },
                                                             set: { accent in context.send(viewAction: .setWhistlepigAccent(accent)) }))
            })
...
private struct WhistlepigAccentPickerRow: View {
    @Binding var selection: WhistlepigAccent
    ...
    var body: some View {
        HStack(spacing: ListRowPadding.labelIconSpacing) {
            Image(systemSymbol: .paintpalette)
                ...
                .frame(width: Compound.supportsGlass ? 24 : 30, height: Compound.supportsGlass ? 24 : 30)
                .padding(.vertical, Compound.supportsGlass ? -1 : -4)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(WhistlepigAccent.allCases) { accent in
                        Button {
                            ...
                            .overlay {
                                Circle().stroke(selection == accent ? Color.compound.textPrimary.opacity(0.55) : .clear, lineWidth: 1.5)
                                    .padding(-3)
                            }
                        }
                        .buttonStyle(.plain)
