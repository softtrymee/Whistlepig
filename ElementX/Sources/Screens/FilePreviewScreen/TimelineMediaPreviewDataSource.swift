...
class TimelineMediaPreviewDataSource: NSObject, QLPreviewControllerDataSource {
    ...
    func previewController(_ controller: QLPreviewController, previewItemAt index: Int) -> any QLPreviewItem {
        ...
        } else if !previewItems.indices.contains(arrayIndex) {
            // A timeline update can briefly leave QuickLook asking for an index from the old
            // padded range. Never subscript the array in that window; return a recoverable loading
            // page and let the pagination publisher reconcile the range.
            MXLog.error("Media preview requested an out-of-bounds item: index=\(index), arrayIndex=\(arrayIndex), count=\(previewItems.count), padding=\(backwardPadding)")
            return TimelineMediaPreviewItem.Loading(state: .paginating(.forwards))
        } else {
...
enum TimelineMediaPreviewItem: Equatable {
    ...
    @Observable class Media: NSObject, QLPreviewItem, Identifiable {
        ...
        /// local echo is replaced by its remote event.
        fileprivate(set) var id: MediaPreviewItemID

        /// Whether this is the local echo that `sentItem` is the remote echo of. The SDK keeps the
        /// timeline unique ID when it swaps a transaction ID for an event ID, so matching on that
        /// ID lets the open preview retain its file handle and download state.
        fileprivate func isLocalEcho(of sentItem: Media) -> Bool {
            guard timelineItem.id.transactionID != nil,
                  sentItem.timelineItem.id.eventID != nil,
            ...
            case (.timelineItem, .timelineItem):
                return true
            case (.galleryItem(_, let item), .galleryItem(_, let sentGalleryItem)):
                return item.id.mediaIndex == sentGalleryItem.id.mediaIndex
            default:
                return false
            }
        }

        /// QuickLook only reads `previewItemURL` when it builds the page. If the file is still
        /// downloading, give it a real local image URL so the page is created as a loading page
        /// and can later be refreshed when the file arrives. Passing nil makes QuickLook build an
        /// unrecoverable "content unavailable" page and is particularly crash-prone when the
        /// caption overlay is being laid out at the same time.
        static let loadingPlaceholderURL: URL? = {
            let size = CGSize(width: 1080, height: 1920)
            let format = UIGraphicsImageRendererFormat()
            format.scale = 1
            let data = UIGraphicsImageRenderer(size: size, format: format).jpegData(withCompressionQuality: 0.5) { rendererContext in
                UIColor.black.setFill()
                rendererContext.fill(CGRect(origin: .zero, size: size))
            }
            let url = FileManager.default.temporaryDirectory
                .appendingPathComponent("media-preview-loading-\(ProcessInfo.processInfo.processIdentifier).jpg")
            do {
                try data.write(to: url, options: .atomic)
                return url
            } catch {
                MXLog.error("Failed writing the media viewer loading placeholder: \(error)")
                return nil
            }
        }()
        ...
        private func updatePreviewItemValues() {
            let url = fileHandle?.url ?? Self.loadingPlaceholderURL
