...
class TimelineTableViewController: UIViewController {
    ...
    /// Recognises taps on the timeline canvas while leaving timeline item cells to
    /// their own native/SwiftUI interactions.
    private lazy var selectionDismissTapGesture: UITapGestureRecognizer = {
        let gesture = UITapGestureRecognizer(target: self, action: #selector(dismissTextSelectionIfNeeded(_:)))
    ...
    init(coordinator: TimelineViewRepresentable.Coordinator,
        ...
        tableView.addGestureRecognizer(selectionDismissTapGesture)
    ...
    @objc private func dismissTextSelectionIfNeeded(_ gesture: UITapGestureRecognizer) {
        guard gesture.state == .ended,
              coordinator.context.textSelectionItemID != nil else {
            return
        }

        // The recognizer is only allowed to receive touches outside TimelineItemCell,
        // so clearing this state cannot interrupt a bubble's links, gestures, or the
        // selected UITextView's native text selection.
        coordinator.context.textSelectionItemID = nil
    }

    private func isInsideTimelineItemCell(_ view: UIView?) -> Bool {
        var current = view
        while let candidate = current {
            if candidate is TimelineItemCell {
                return true
            }
            ...
            case TimelineItemIdentifier.UniqueID(TimelineTypingIndicatorCell.reuseIdentifier):
                ...
                cell.contentConfiguration = UIHostingConfiguration {
                    TypingIndicatorView(typingMembers: self.typingMembers)
                        // Keep the visible line 4pt above the composer. The initial empty
                        // state uses a 1pt transparent placeholder, so its padding is 3pt.
                        .padding(.bottom, self.typingMembers.members.isEmpty ? 3 : 4)
                }
                .margins(.vertical, 0)
                .minSize(height: 0)
...
extension TimelineTableViewController: UIGestureRecognizerDelegate {
    func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        guard gestureRecognizer === selectionDismissTapGesture else { return true }

        // Do not compete with any timeline item cell. This includes the focused
        // UITextView (double-tap/long-press/selection handles) and all other
        // bubble links, reactions, replies, and media gestures.
        return !isInsideTimelineItemCell(touch.view)
    }
}
