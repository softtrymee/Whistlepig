...
private extension TimelineItemSendInfo {
    init(timelineItem: EventBasedTimelineItemProtocol,
        ...
        // Live activity cards already communicate their phase in the card itself. Keep the
        // timestamp, but don't repeat the generic `(edited)` marker for their streaming edits.
        localizedString = timelineItem.isWhistlepigAgentActivity
            ? timelineItem.timestamp.formattedTime()
            : timelineItem.localizedSendInfo
        ...
        } else if timelineItem.isWhistlepigAgentActivity {
            // The activity card owns an animated border; overlaying the timestamp at the
            // bottom-trailing lets the roaming highlight cross it. Put the time on its own
            // line below the card instead.
            .vertical()
        } else {
            switch timelineItem {
...
private extension EventBasedTimelineItemProtocol {
    var isWhistlepigAgentActivity: Bool {
        switch self {
        case let item as TextRoomTimelineItem:
            item.liveActivityBody != nil
