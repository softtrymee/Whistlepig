...
class TimelineViewModel: TimelineViewModelType, TimelineViewModelProtocol {
    ...
    override func process(viewAction: TimelineViewAction) {
        ...
        case .displayTimelineItemMenu(let itemID):
            // Opening a menu for another item exits any previous native text selection.
            if state.bindings.textSelectionItemID != itemID {
                state.bindings.textSelectionItemID = nil
            }
        ...
        case .handleTimelineItemMenuAction(let itemID, let action):
            switch action {
            case .selectText:
                // TimelineItemMenu dismisses itself before this action reaches the view model.
                // Publishing the item ID here lets the rendered cell enable native UITextView
                // selection without coupling that UI state to TimelineInteractionHandler.
                state.bindings.textSelectionItemID = itemID
            case .copy, .copyCaption:
                state.bindings.textSelectionItemID = nil
    ...
    private func showStickerPicker() {
        let viewModel = StickerPickerViewModel(roomProxy: roomProxy,
        ...
        viewModel.actions
            .sink { [weak self] action in
                ...
                switch action {
                case .dismiss:
                    state.bindings.stickerPickerViewModel = nil
                case .sendSticker(let sticker):
                    Task { await self.sendSticker(sticker) }
                }
            }
            .store(in: &cancellables)
        state.bindings.stickerPickerViewModel = viewModel
    }
    
    private func sendSticker(_ sticker: StickerPickerSticker) async {
        switch await roomProxy.sendSticker(content: sticker.content) {
        case .success:
            state.bindings.stickerPickerViewModel = nil
        case .failure:
            displayErrorToast(L10n.commonFailed)
    ...
    private func buildTimelineViews(timelineItems: [RoomTimelineItemProtocol], isSwitchingTimelines: Bool = false) {
        ...
        if let selectedItemID = state.bindings.textSelectionItemID,
           !timelineItems.contains(where: { $0.id == selectedItemID }) {
            state.bindings.textSelectionItemID = nil
        }
    }
