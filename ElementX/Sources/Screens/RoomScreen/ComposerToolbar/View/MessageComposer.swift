...
struct MessageComposer: View {
    ...
    /// The leading inset is configurable so the toolbar can bring the text cursor
    /// closer to its leading accessory without changing either control's hit area.
    let composerLeadingPadding: CGFloat?
    ...
    init(plainComposerText: Binding<NSAttributedString>,
         presendCallback: Binding<(() -> Void)?>,
         selectedRange: Binding<NSRange>,
         composerView: WysiwygComposerView,
         mode: ComposerMode,
         placeholder: String,
         composerFormattingEnabled: Bool,
         showResizeGrabber: Bool,
         isExpanded: Binding<Bool>,
         composerLeadingPadding: CGFloat? = nil,
         sendAction: @escaping () -> Void,
         editAction: @escaping () -> Void,
         pasteAction: @escaping PasteHandler,
         cancellationAction: @escaping () -> Void,
         onAppearAction: @escaping () -> Void) {
        _plainComposerText = plainComposerText
        _presendCallback = presendCallback
        _selectedRange = selectedRange
        self.composerView = composerView
        self.mode = mode
        self.placeholder = placeholder
        self.composerFormattingEnabled = composerFormattingEnabled
        self.showResizeGrabber = showResizeGrabber
        _isExpanded = isExpanded
        self.composerLeadingPadding = composerLeadingPadding
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            composerTextField
                .messageComposerStyle(header: header, leadingPadding: composerLeadingPadding)
...
extension View {
    func messageComposerStyle(header: some View = EmptyView(), leadingPadding: CGFloat? = nil) -> some View {
        modifier(MessageComposerStyleModifier(header: header, leadingPadding: leadingPadding))
    }
}
...
private struct MessageComposerStyleModifier<Header: View>: ViewModifier {
    ...
    func mainContent(content: Content) -> some View {
        ...
        .padding(.leading, leadingPadding ?? (Compound.supportsGlass ? 16 : 12))
        .padding(.trailing, Compound.supportsGlass ? 16 : 12)
    }
}
