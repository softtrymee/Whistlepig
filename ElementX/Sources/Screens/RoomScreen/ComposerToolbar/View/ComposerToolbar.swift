...
struct ComposerToolbar: View {
    ...
    var body: some View {
        ...
        .padding(.leading, 12)
        .padding(.trailing, 12)
        .padding(.bottom, context.composerFormattingEnabled ? 6 : 6)
        .background {
            if context.composerFormattingEnabled {
    ...
    private var topBar: some View {
        ...
        .animation(.spring(response: 0.22, dampingFraction: 0.86), value: isTyping)
        .padding(6)
        .whistlepigGlassSurface(RoundedRectangle(cornerRadius: 24, style: .continuous), interactive: true)
    }
    ...
    private var isTyping: Bool {
        context.composerFocused || !context.plainComposerText.string.isEmpty
    }
    
    private var topBarLayout: some Layout {
        HStackLayout(alignment: .bottom, spacing: isTyping ? 2 : 12)
    }
    
    private var mainTopBarContent: some View {
        ZStack(alignment: .bottom) {
            HStack(alignment: .bottom, spacing: isTyping ? 2 : 12) {
                if !context.composerFormattingEnabled {
                    HStack(spacing: isTyping ? 0 : 12) {
                        RoomAttachmentPicker(context: context)
                            .scaledPadding(.vertical, buttonVerticalPadding, relativeTo: .compound.headingLG)
                            .scaleEffect(isTyping ? 0.78 : 1, anchor: .leading)
                        StickerPickerButton(context: context)
                            .scaledPadding(.vertical, buttonVerticalPadding, relativeTo: .compound.headingLG)
                            .scaleEffect(isTyping ? 0.78 : 1, anchor: .leading)
                    }
                }
    ...
    private var sendButton: some View {
        SendButton(mode: context.viewState.sendButtonMode,
                   backgroundColor: WhistlepigTheme.ColorToken.accentPrimary,
                   action: sendMessage)
    ...
    private var messageComposer: some View {
        MessageComposer(plainComposerText: $context.plainComposerText,
                        ...
                        composerLeadingPadding: Compound.supportsGlass ? 8 : 6,
                        sendAction: {
                            sendMessage()
                        },
                        editAction: {
