...
final class CompletionSuggestionService: CompletionSuggestionServiceProtocol {
    ...
    private func detectTriggerInText(_ text: String, selectedRange: NSRange) -> SuggestionTrigger? {
        ...
        if firstChar == SuggestionTriggerRegex.at,
           match.range.lowerBound > text.startIndex {
            let precedingCharacter = text[text.index(before: match.range.lowerBound)]
            guard !precedingCharacter.isLetter,
                  !precedingCharacter.isNumber,
                  !"._+-".contains(precedingCharacter) else {
                return nil
            }
        }
