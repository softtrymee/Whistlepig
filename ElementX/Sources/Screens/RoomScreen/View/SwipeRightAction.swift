...
struct SwipeLeftAction<Label: View>: ViewModifier {
...
extension View {
    func swipeLeftAction(label: @escaping () -> some View,
                         shouldStartAction: @escaping () -> Bool,
                         action: @escaping () -> Void) -> some View {
        modifier(SwipeLeftAction(label: label, shouldStartAction: shouldStartAction, action: action))
    }
}

struct SwipeLeftAction_Previews: PreviewProvider, TestablePreview {
...
private struct PanGestureRepresentable: UIGestureRecognizerRepresentable {
    ...
    class Coordinator: NSObject, UIGestureRecognizerDelegate {
        ...
        func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
            ...
            return velocity.x < 0 && abs(velocity.y) < abs(velocity.x)
        }
    }
}
