...
private struct NavigationTabCoordinatorView<Tag: Hashable>: View {
    ...
    var tabView: some View {
        if navigationTabCoordinator.tabModules.count == 1 {
            singleTabLayout
        } else if horizontalSizeClass == .regular, #available(iOS 26, *) {
            tabRailLayout
        } else {
            tabViewLayout
        }
    ...
    @ViewBuilder
    private var singleTabLayout: some View {
        if let module = navigationTabCoordinator.tabModules.first {
            module.coordinator?.toPresentable()
                .id(module.id)
        }
    }
