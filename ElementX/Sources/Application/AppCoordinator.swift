...
class AppCoordinator: AppCoordinatorProtocol, AuthenticationFlowCoordinatorDelegate, NotificationManagerDelegate, SecureWindowManagerDelegate {
    ...
    init(appDelegate: AppDelegate) {
        ...
        } else if !userSessionStore.hasSessions {
            /*
             The app has been deleted since the previous run. Reset any remaining settings.
             A Personal Team build has no App Group entitlement, so an upgrade can lose the
             old App Group version marker while the app container and Keychain still hold a
             valid Matrix session. Preserve that session long enough to validate and restore it.
             */
            wipeUserData(includingSettings: true)
        } else {
            MXLog.info("No version marker found; preserving the existing session for restoration.")
        }
    ...
    func toPresentable() -> AnyView {
        AnyView(WhistlepigThemeHost(appSettings: appSettings) {
