...
final class MessageTextView: UITextView, PillAttachmentViewProviderDelegate, UIGestureRecognizerDelegate {
    ...
    /// Makes this text view ready for the system's native selection gestures.
    ///
    /// A read-only UITextView does not create a caret just because it becomes the first
    /// responder. Giving it a collapsed range also makes a subsequent native long press start
    /// from a real insertion point instead of being ignored by TextKit.
    func activateTextSelection() {
        guard isSelectable, !textStorage.string.isEmpty else { return }
        ...
        let position = beginningOfDocument
        selectedTextRange = textRange(from: position, to: position)
    }

    /// Returns the rendered text for the current selection, replacing TextKit's attachment
    /// placeholder characters with useful clipboard text. In particular, the timestamp spacer
    /// is intentionally omitted and mention pills fall back to their Matrix identifier.
    func selectedTextForCopy() -> String? {
        ...
        textStorage.enumerateAttributes(in: selectedRange, options: []) { attributes, range, _ in
            guard let attachment = attributes[.attachment] as? NSTextAttachment else {
                result.append(textStorage.attributedSubstring(from: range).string)
                return
            }

            if let pillAttachment = attachment as? PillTextAttachment,
               let pillData = pillAttachment.pillData {
                ...
                case .event(let room):
                    ...
                    case .roomAlias(let roomAlias):
                        result.append(roomAlias)
                    case .roomID(let roomID):
                        result.append(roomID)
                    }
                }
            }
            // Transparent timestamp attachments and unknown attachments are omitted.
        }
        return result as String
    }

    override func copy(_ sender: Any?) {
        if let selectedText = selectedTextForCopy() {
...
struct MessageText: UIViewRepresentable {
    ...
    @Environment(\.messageTextSelectionEnabled) private var selectionEnabled
    ...
    func updateUIView(_ uiView: MessageTextView, context: Context) {
        context.coordinator.selectionEnabled = selectionEnabled
        // Caption previews can be mounted before their SwiftUI environment has finished
        // propagating through the UIKit representable. Keep the provider's delegate context in
        // sync on every update, not only when the UITextView is first created.
        uiView.timelineContext = viewModel
        ...
        if selectionEnabled {
            // The menu sheet is dismissed before this state is published. Deferring one run loop
            // gives the cell time to be on screen while still using UITextView's native handles
            // and edit menu. A collapsed range is needed for read-only UITextView to expose its
            // caret and to let a native long press begin selection.
            DispatchQueue.main.async { [weak uiView] in
                guard let uiView, context.coordinator.selectionEnabled else { return }
                uiView.activateTextSelection()
            }
...
private struct MessageTextSelectionEnabledKey: EnvironmentKey {
    static let defaultValue = false
}

extension EnvironmentValues {
    var messageTextSelectionEnabled: Bool {
        get { self[MessageTextSelectionEnabledKey.self] }
        set { self[MessageTextSelectionEnabledKey.self] = newValue }
    }
}
