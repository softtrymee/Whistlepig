...
final nonisolated class PillTextAttachment: NSTextAttachment {
    ...
    /// The attachment can be reconstructed by Foundation without calling our convenience
    /// initializer (for example when an attributed string is copied into another TextKit view).
    /// Keep this optional so a stale/corrupt attachment degrades to normal attachment metrics
    /// instead of trapping while TextKit lays out a caption.
    ...
    override func attachmentBounds(for textContainer: NSTextContainer?, proposedLineFragment lineFrag: CGRect, glyphPosition position: CGPoint, characterIndex charIndex: Int) -> CGRect {
        ...
        guard let fontData = pillData?.fontData else {
            return rect
        }
