...
struct LoadableImage<TransformerView: View, PlaceholderView: View>: View {
    ...
    private let detectsAnimatedImage: Bool
    ...
    init(mediaSource: MediaSourceProxy,
         ...
         detectsAnimatedImage: Bool = false,
         blurhash: String? = nil,
         size: CGSize? = nil,
        ...
        self.detectsAnimatedImage = detectsAnimatedImage
    ...
    var body: some View {
        if let mediaSource {
            LoadableImageContent(mediaSource: mediaSource,
                                 mediaType: mediaType,
                                 detectsAnimatedImage: detectsAnimatedImage,
                                 blurhash: blurhash,
                                 size: size,
...
private struct LoadableImageContent<TransformerView: View, PlaceholderView: View>: View {
    ...
    var body: some View {
        ZStack {
            ...
            case (.animatedImageData(let data), true):
                transformer(AnyView(AnimatedStickerImage(data: data)))
            case (.none, _), (_, false):
                if let blurHashView {
                    if shouldRender {
        ...
        .task(id: mediaSource.url.absoluteString + "\(shouldRender)") {
            ...
            guard contentLoader.content == nil else { return }
            
            let didLoad = await contentLoader.load()
...
struct AnimatedStickerImage: UIViewRepresentable {
    ...
    private static func registerWebPCoder() {
...
private class ContentLoader: ObservableObject {
    ...
    init(mediaSource: MediaSourceProxy, size: CGSize?, detectsAnimatedImage: Bool, mediaProvider: MediaProviderProtocol?) {
        self.mediaSource = mediaSource
        self.size = size
        self.detectsAnimatedImage = detectsAnimatedImage
        self.mediaProvider = mediaProvider
    ...
    func load() async -> Bool {
        if loadsAnimatedImageData {
            ...
            return await loadThumbnailFallback()
        } else {
    ...
    private var loadsAnimatedImageData: Bool {
        detectsAnimatedImage || isGIF
    }
    ...
    private func isAnimatedImage(_ data: Data) -> Bool {
    ...
    private func setContent(from data: Data) {
        if detectsAnimatedImage,
           !isAnimatedImage(data),
           let image = UIImage(data: data) {
            cachedContent = .image(image)
        } else {
            cachedContent = .animatedImageData(data)
        }
    }
    
    private func loadThumbnailFallback() async -> Bool {
        guard let size,
              !Task.isCancelled,
              case let .success(data) = await mediaProvider?.loadThumbnailForSource(source: mediaSource, size: size),
