...
struct RoundedCornerShape: Shape {
    ...
    var compactRadius: CGFloat = 0
    var compactCorners: UIRectCorner = []
    ...
    func path(in rect: CGRect) -> Path {
        ...
        var topLeft = cornerRadius(for: .topLeft)
        var topRight = cornerRadius(for: .topRight)
        var bottomLeft = cornerRadius(for: .bottomLeft)
        var bottomRight = cornerRadius(for: .bottomRight)
    ...
    private func cornerRadius(for corner: UIRectCorner) -> CGFloat {
        if compactCorners.contains(corner) {
            compactRadius
        } else if corners.contains(corner) {
            radius
        } else {
            0
        }
