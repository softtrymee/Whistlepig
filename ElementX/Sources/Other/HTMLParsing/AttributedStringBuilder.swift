...
nonisolated struct AttributedStringBuilder: AttributedStringBuilderProtocol {
    ...
    func attributedString(element: Element,
        ...
        for node in element.getChildNodes() {
            ...
            case "table":
                if let table = tableModel(for: childElement,
                                          documentBody: documentBody,
                                          preserveFormatting: preserveFormatting,
                                          indentLevel: indentLevel) {
                    // The marker is deliberately zero-width. It gives the table a stable
                    // position in the attributed string without leaking a replacement glyph
                    // into previews or accessibility text.
                    content = NSMutableAttributedString(string: "\u{200B}")
                    content.addAttribute(.MatrixTable,
                                         value: table,
                                         range: NSRange(location: 0, length: content.length))
                } else {
                    // Malformed tables are rendered using the existing text fallback. In
                    // particular, this keeps nested tables and rows with missing cells from
                    // crashing the timeline or silently losing their text.
                    content = attributedString(element: childElement,
                                               documentBody: documentBody,
                                               preserveFormatting: preserveFormatting,
                                               listTag: listTag,
    ...
    /// Parses the supported Matrix HTML table subset into a model used by the SwiftUI renderer.
    ///
    /// Only direct rows (optionally grouped in thead/tbody/tfoot) are accepted. A rectangular
    /// table is required so that every renderer can safely calculate column widths. Anything
    /// outside this subset is left to the normal HTML text fallback.
    private func tableModel(for table: Element,
                            documentBody: Element,
                            preserveFormatting: Bool,
                            indentLevel: Int) -> TableAttribute.Value? {
        var rows = [[Element]]()
        var captionElement: Element?

        for node in table.getChildNodes() {
            if let textNode = node as? TextNode {
                guard textNode.isBlank() else { return nil }
                continue
            }
            ...
            case "caption":
                guard captionElement == nil else { return nil }
                captionElement = element
            case "tr":
                guard let row = tableRow(from: element) else { return nil }
                rows.append(row)
            case "thead", "tbody", "tfoot":
                for sectionNode in element.getChildNodes() {
                    if let textNode = sectionNode as? TextNode {
                        guard textNode.isBlank() else { return nil }
                        continue
                    }
                    guard let rowElement = sectionNode as? Element,
                          rowElement.tagName().lowercased() == "tr",
                          let row = tableRow(from: rowElement) else {
                        return nil
                    }
                    rows.append(row)
                }
        ...
        let caption = captionElement.flatMap { element -> AttributedString? in
            var listIndex = 1
            let content = attributedString(element: element,
                                           documentBody: documentBody,
                                           preserveFormatting: preserveFormatting,
                                           listTag: nil,
                                           listIndex: &listIndex,
                                           indentLevel: indentLevel)
            trimTrailingNewlines(from: content)
            addLinksAndMentions(content)
            addMatrixEntityPermalinkAttributesTo(content)
            detectPhishingAttempts(content)
            return try? AttributedString(content, including: \.elementX)
        }

        let parsedRows = rows.compactMap { row -> TableAttribute.Row? in
            let cells = row.compactMap { cell -> TableAttribute.Cell? in
                guard let cellContent = tableCellContent(for: cell,
                                                         documentBody: documentBody,
                                                         preserveFormatting: preserveFormatting,
                                                         indentLevel: indentLevel),
                    let value = try? AttributedString(cellContent, including: \.elementX) else {
                    return nil
                }
                return TableAttribute.Cell(attributedString: value,
                                           isHeader: cell.tagName().lowercased() == "th")
            }
            guard cells.count == row.count else { return nil }
            return TableAttribute.Row(cells: cells)
        }

        guard parsedRows.count == rows.count else { return nil }
        return TableAttribute.Value(caption: caption, rows: parsedRows)
    }

    private func tableRow(from element: Element) -> [Element]? {
        var cells = [Element]()
        for node in element.getChildNodes() {
            if let textNode = node as? TextNode {
                guard textNode.isBlank() else { return nil }
            ...
            guard let cell = node as? Element else { return nil }
            let tag = cell.tagName().lowercased()
            guard tag == "th" || tag == "td" else { return nil }
            guard (try? cell.getElementsByTag("table").isEmpty) == true else { return nil }
            guard (try? cell.attr("colspan")).map({ $0.isEmpty || $0 == "1" }) ?? true,
                  (try? cell.attr("rowspan")).map({ $0.isEmpty || $0 == "1" }) ?? true else {
                return nil
            }
            cells.append(cell)
        }
    ...
    private func tableCellContent(for cell: Element,
                                  documentBody: Element,
                                  preserveFormatting: Bool,
                                  indentLevel: Int) -> NSMutableAttributedString? {
        var listIndex = 1
        let content = attributedString(element: cell,
                                       documentBody: documentBody,
                                       preserveFormatting: preserveFormatting,
                                       listTag: nil,
                                       listIndex: &listIndex,
    ...
    private func trimTrailingNewlines(from attributedString: NSMutableAttributedString) {
        while attributedString.string.last?.isNewline == true {
