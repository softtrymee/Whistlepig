...
class UserSessionStore: UserSessionStoreProtocol {
    ...
    private func restorePreviousLogin(_ credentials: KeychainCredentials) async -> Result<ClientProxyProtocol, UserSessionStoreError> {
        let restorationToken = credentials.restorationToken
        let sessionDirectories = restorationToken.sessionDirectories.rebasedToCurrentContainer()
        guard sessionDirectories.isNonTransientUserDataValid() else {
            MXLog.error("Failed restoring login, missing non-transient user data")
            return .failure(.failedRestoringLogin)
        }
        
        let homeserverURL = restorationToken.session.homeserverUrl
        ...
        let builder = ClientBuilder
            ...
            .sqliteStore(config: .init(dataPath: sessionDirectories.dataPath,
                                       cachePath: sessionDirectories.cachePath)
                    .passphrase(passphrase: restorationToken.passphrase))
            .withSearchIndexStore(path: sessionDirectories.dataPath,
                                  password: restorationToken.passphrase)
            .homeserverUrl(url: homeserverURL)
        
        do {
            let client = try await builder.build()
            try await client.restoreSession(session: restorationToken.session)
            keychainController.setRestorationToken(RestorationToken(session: restorationToken.session,
                                                                    sessionDirectories: sessionDirectories,
                                                                    passphrase: restorationToken.passphrase,
                                                                    pusherNotificationClientIdentifier: restorationToken.pusherNotificationClientIdentifier),
                                                   forUsername: credentials.userID)
            
            MXLog.info("Set up session for user \(credentials.userID) at: \(sessionDirectories)")
