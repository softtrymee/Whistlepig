...
nonisolated struct SessionDirectories: Hashable, Codable {
    ...
    /// Recreates session paths under the current app container. iOS may change the
    /// data-container UUID during an in-place app upgrade while retaining its files.
    func rebasedToCurrentContainer() -> SessionDirectories {
        .init(dataDirectory: .sessionsBaseDirectory.appending(component: dataDirectory.lastPathComponent, directoryHint: .isDirectory),
              cacheDirectory: .sessionCachesBaseDirectory.appending(component: cacheDirectory.lastPathComponent, directoryHint: .isDirectory))
    }
