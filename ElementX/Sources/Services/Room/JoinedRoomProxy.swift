...
class JoinedRoomProxy: JoinedRoomProxyProtocol {
    ...
    func stateEventsRaw(eventType: String) async -> Result<[String], RoomProxyError> {
        do {
            return try await .success(room.stateEventsRaw(eventType: eventType))
        } catch {
            MXLog.error("Failed retrieving state events of type \(eventType) with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    
    func sendSticker(content: String) async -> Result<Void, RoomProxyError> {
        do {
            _ = try await room.sendRaw(eventType: "m.sticker", content: content)
            return .success(())
        } catch {
