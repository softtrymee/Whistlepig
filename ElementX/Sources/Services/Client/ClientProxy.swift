...
class ClientProxy: ClientProxyProtocol {
    ...
    func accountData(eventType: String) async -> Result<String?, ClientProxyError> {
        do {
            ...
            MXLog.error("Failed retrieving account data event \(eventType) with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    
    func fetchAccountData(eventType: String) async -> Result<String?, ClientProxyError> {
        do {
            return try await .success(client.fetchAccountData(eventType: eventType))
        } catch {
            MXLog.error("Failed fetching account data event \(eventType) with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    
    func fetchStateEventsRaw(roomID: String) async -> Result<[String], ClientProxyError> {
        do {
            return try await .success(client.fetchStateEventsRaw(roomId: roomID))
        } catch {
            MXLog.error("Failed fetching room state for \(roomID) with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
