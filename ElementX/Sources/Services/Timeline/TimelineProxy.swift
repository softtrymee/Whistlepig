...
final class TimelineProxy: TimelineProxyProtocol {
    ...
    /// Thread timelines do not expose pagination status updates from the SDK. Keep the
    /// initial backwards preload owned by the proxy so the table view cannot start a second
    /// request while the first batch is still being fetched.
    private var threadPreloadTask: Task<Void, Never>?
    ...
    func subscribeForUpdates() async {
        ...
        if kind.isThread {
            MXLog.info("Thread timeline created; initial item count: \(provider.itemProxies.count)")
            threadPreloadTask = Task { [weak self] in
                await self?.preloadInitialThreadReplies()
            }
        }
    ...
    /// The SDK's thread timeline initially exposes the newest reply, while older replies are
    /// available through backwards pagination. Since thread timelines do not provide pagination
    /// status callbacks, mark the direction as busy before publishing the provider and perform
    /// one bounded preload. Subsequent table-driven pagination remains responsible for older
    /// pages once this request completes.
    private func preloadInitialThreadReplies() async {
        MXLog.info("Thread initial backwards preload started")
        do {
            let reachedStart = try await timeline.paginateBackwards(numEvents: 20)
            backPaginationStateSubject.send(reachedStart ? .endReached : .idle)
            MXLog.info("Thread initial backwards preload finished; item count: \(innerTimelineItemProvider?.itemProxies.count ?? 0)")
        } catch {
            backPaginationStateSubject.send(.idle)
            MXLog.error("Thread initial backwards preload failed")
        }
    }
    ...
    private func subscribeToPagination() async {
        ...
        case .thread:
            // Thread timelines don't support observation. Reserve backwards pagination for the
            // initial preload before publishing the provider, and avoid a simultaneous forward
            // request while the newest reply is already present.
