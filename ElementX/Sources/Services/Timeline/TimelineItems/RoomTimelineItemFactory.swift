...
nonisolated struct RoomTimelineItemFactory: RoomTimelineItemFactoryProtocol {
    ...
    func buildTimelineItem(for eventItemProxy: EventTimelineItemProxy, isDM: Bool) -> RoomTimelineItemProtocol? {
        ...
        case .msgLike(let messageLikeContent):
            ...
            case .liveLocation:
                // Location events are intentionally not rendered by Whistlepig.
                // Keep historical events visible as a harmless unsupported item.
                return buildUnsupportedTimelineItem(eventItemProxy, "m.location", L10n.commonUnsupportedEvent, isOutgoing)
            case .other:
    ...
    private func buildTextTimelineItem(for eventItemProxy: EventTimelineItemProxy,
        ...
        TextRoomTimelineItem(id: eventItemProxy.id,
                             ...
                             liveActivityBody: liveActivityContent(for: eventItemProxy)?.body,
                             liveActivityFormattedBody: liveActivityContent(for: eventItemProxy)?.formattedBody,
                             liveActivityCompleted: liveActivityCompleted(for: eventItemProxy),
    ...
    // MARK: - MSC4357 streaming activity

    /// Returns the already aggregated display body only for a root event whose original
    /// (decrypted, when applicable) content explicitly opted into MSC4357 live updates.
    /// A stream is an activity while its root has no terminal replacement, or while the
    /// latest replacement still carries the MSC4357 marker. Once a replacement explicitly
    /// removes the marker, return nil so the normal formatted/text body is rendered.
    private struct LiveActivityContent {
        let body: String
        let formattedBody: AttributedString?
    }

    private func liveActivityContent(for eventItemProxy: EventTimelineItemProxy) -> LiveActivityContent? {
        let debugInfo = eventItemProxy.debugInfo
        let originalHasLiveMarker = Self.containsLiveMarker(in: debugInfo.originalJSON)
        let latestEditHasLiveMarker = Self.containsLiveMarker(in: debugInfo.latestEditJSON)
        let hasTerminalReplacement: Bool
        if originalHasLiveMarker,
           let rootID = Self.eventID(in: debugInfo.originalJSON),
           let latestEdit = debugInfo.latestEditJSON {
            hasTerminalReplacement = Self.isReplacement(latestEdit, of: rootID) &&
                !latestEditHasLiveMarker
        } else {
            hasTerminalReplacement = false
        }

        // MSC4357 is authoritative. Without its marker we cannot distinguish a
        // live stream from an ordinary notice/edit, so leave the latter alone.
        let isActivity = !hasTerminalReplacement &&
            (originalHasLiveMarker || latestEditHasLiveMarker)
        Self.logLiveMarkerDiagnostics(eventItemProxy: eventItemProxy,
                                      originalHasLiveMarker: originalHasLiveMarker,
                                      latestEditHasLiveMarker: latestEditHasLiveMarker,
                                      hasLatestEdit: debugInfo.latestEditJSON != nil)

        guard isActivity else {
            return nil
        }

        // The Rust timeline item exposes the original event's parsed content, while
        // debugInfo.latestEditJSON is the authoritative newest m.replace payload. Read
        // that payload here so each replacement naturally changes the SwiftUI value.
        if let latestEdit = debugInfo.latestEditJSON,
           let latest = Self.messageContent(in: latestEdit) {
            return LiveActivityContent(body: latest.body,
                                       formattedBody: latest.htmlBody.flatMap(attributedStringBuilder.fromHTML))
        }

        guard let body = Self.messageBody(for: eventItemProxy) else { return nil }
        return LiveActivityContent(body: body,
                                   formattedBody: Self.messageHTMLBody(for: eventItemProxy)
            .flatMap(attributedStringBuilder.fromHTML))
    }

    private func liveActivityCompleted(for eventItemProxy: EventTimelineItemProxy) -> Bool {
        let debugInfo = eventItemProxy.debugInfo
        if Self.containsLiveMarker(in: debugInfo.originalJSON) {
            guard let rootID = Self.eventID(in: debugInfo.originalJSON),
                  let latestEdit = debugInfo.latestEditJSON,
                  Self.isReplacement(latestEdit, of: rootID) else {
                return false
            }
            return !Self.containsLiveMarker(in: latestEdit)
        }
        return false
    }

    private static let liveMarker = "org.matrix.msc4357.live"

    private static func eventID(in json: String?) -> String? {
        guard let json,
        ...
        return ((event["event"] as? [String: Any]) ?? event)["event_id"] as? String
    }

    private static func isReplacement(_ json: String, of rootID: String) -> Bool {
        guard let data = json.data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data),
              let rawEvent = object as? [String: Any] else {
            return false
        }
        let event = (rawEvent["event"] as? [String: Any]) ?? rawEvent
        let content = (event["content"] as? [String: Any]) ?? event
        let relation = content["m.relates_to"] as? [String: Any]
        return relation?["rel_type"] as? String == "m.replace" &&
            relation?["event_id"] as? String == rootID
    }

    private static func containsLiveMarker(in json: String?) -> Bool {
        guard let json,
              let data = json.data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data),
              let event = object as? [String: Any] else {
        ...
        // The SDK exposes a complete AnySyncTimelineEvent. Keep the content-only fallback
        // for SDK versions that expose the raw content instead. OpenClaw v2026.9.2 follows
        // MSC4357's object marker form (`"org.matrix.msc4357.live": {}`). Accept that
        // form as well as a literal true, but never treat an explicit false as live.
        let content = (event["content"] as? [String: Any]) ?? event
        return markerIsTrue(in: content) ||
            ((content["m.new_content"] as? [String: Any]).map { markerIsTrue(in: $0) } ?? false)
    }

    private static func markerIsTrue(in content: [String: Any]) -> Bool {
        guard let marker = content[liveMarker] else {
            return false
        }

        if let bool = marker as? Bool {
            return bool
        }

        return marker is [String: Any]
    }

    private static func messageBody(for eventItemProxy: EventTimelineItemProxy) -> String? {
        guard case .msgLike(let content) = eventItemProxy.content,
              case .message(let message) = content.kind else {
            return nil
        }

        switch message.msgType {
        case .text(let value): return value.body
        case .notice(let value): return value.body
        default: return nil
        }
    }

    private static func messageHTMLBody(for eventItemProxy: EventTimelineItemProxy) -> String? {
        guard case .msgLike(let content) = eventItemProxy.content,
              case .message(let message) = content.kind else { return nil }
        switch message.msgType {
        case .text(let value): return value.formatted?.format == .html ? value.formatted?.body : nil
        case .notice(let value): return value.formatted?.format == .html ? value.formatted?.body : nil
        default: return nil
        }
    }

    private static func messageContent(in json: String) -> (body: String, htmlBody: String?)? {
        guard let data = json.data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { return nil }
        let event = (object["event"] as? [String: Any]) ?? object
        let content = (event["content"] as? [String: Any]) ?? event
        // Replacement events put the actual message in m.new_content. Supporting
        // content directly also covers SDK/debug builds that already unwrap it.
        let message = (content["m.new_content"] as? [String: Any]) ?? content
        guard let body = message["body"] as? String else { return nil }
        let formatted = message["formatted_body"] as? String
        let format = message["format"] as? String
        return (body, format?.lowercased().contains("html") == true ? formatted : nil)
    }

    #if DEBUG
    /// Metadata-only diagnostics for simulator/OpenClaw interop debugging. Never log event
    /// bodies or raw JSON: this must remain safe even when the event is a private DM.
    private static func logLiveMarkerDiagnostics(eventItemProxy: EventTimelineItemProxy,
                                                 originalHasLiveMarker: Bool,
                                                 latestEditHasLiveMarker: Bool,
                                                 hasLatestEdit: Bool) {
        let relationMode = hasLatestEdit ? "replace" : "original"
        let eventKind: String
        switch eventItemProxy.content {
        case .msgLike: eventKind = "msgLike"
        case .state: eventKind = "state"
        case .failedToParseMessageLike: eventKind = "failedToParseMessageLike"
        case .failedToParseState: eventKind = "failedToParseState"
        case .roomMembership: eventKind = "roomMembership"
        case .profileChange: eventKind = "profileChange"
        case .callInvite: eventKind = "callInvite"
        case .rtcNotification: eventKind = "rtcNotification"
        }
        print("[MSC4357] eventKind=\(eventKind) originalMarker=\(originalHasLiveMarker) latestEditMarker=\(latestEditHasLiveMarker) relationMode=\(relationMode)")
    }
    #endif
