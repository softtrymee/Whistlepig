...
struct AttributedStringBuilderTests {
    ...
    @Test
    func renderHTMLTableAsStructuredComponent() throws {
        let html = """
        <p>Before the table</p>
        <table>
          <thead><tr><th>Plan</th><th>Limit</th><th>Details</th></tr></thead>
          <tbody>
            <tr><td><strong>HSA</strong></td><td>$8,750</td><td><a href=\"https://example.com\">Eligible</a> 🐸</td></tr>
            <tr><td>FSA</td><td>Use it</td><td>Ends in <em>December</em></td></tr>
          </tbody>
        </table>
        <p>After the table</p>
        """
        ...
        #expect(components[0].type == .plainText)
        #expect(components[0].attributedString.string == "Before the table")

        guard case .table(let table) = components[1].type else {
            Issue.record("The HTML table was not parsed as a table component")
            return
        }

        #expect(table.rows.count == 3)
        #expect(table.rows[0].cells.count == 3)
        let allHeaders = table.rows[0].cells.reduce(true) { result, cell in
            result && cell.isHeader
        }
        #expect(allHeaders)
        #expect(table.rows[0].cells[0].attributedString.string == "Plan")
        #expect(table.rows[1].cells[0].attributedString.string == "HSA")
        #expect(table.rows[1].cells[1].attributedString.string == "$8,750")
        #expect(table.rows[1].cells[2].attributedString.runs.contains { $0.link != nil })
        #expect(table.rows[1].cells[2].attributedString.string == "Eligible 🐸")
        #expect(table.rows[1].cells[0].attributedString.runs.contains { $0.uiKit.font?.fontDescriptor.symbolicTraits.contains(.traitBold) == true })
        #expect(table.rows[2].cells[2].attributedString.string == "Ends in December")
        #expect(table.rows[2].cells[2].attributedString.runs.contains { $0.uiKit.font?.fontDescriptor.symbolicTraits.contains(.traitItalic) == true })
        #expect(components[2].attributedString.string == "After the table")
    }

    @Test
    func malformedHTMLTableFallsBackToText() throws {
        let html = "<p>Before</p><table><tr><th>One</th><th>Two</th></tr><tr><td>Only one cell</td></tr></table><p>After</p>"
        let attributedString = try #require(attributedStringBuilder.fromHTML(html), "Could not build the attributed string")

        #expect(attributedString.formattedComponents.allSatisfy {
            if case .table = $0.type {
                return false
            }
            return true
        })
        #expect(attributedString.string.contains("Only one cell"))
    }
