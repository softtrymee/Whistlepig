...
struct TimelineMediaPreviewDataSourceTests {
    ...
    @Test
    func mediaWithoutFileHandleAlwaysHasAQuickLookURL() throws {
        let dataSource = TimelineMediaPreviewDataSource(itemViewStates: initialMediaViewStates,
                                                        initialItem: initialMediaItems[initialItemIndex],
                                                        paginationState: .initial)

        let media = try #require(dataSource.currentItem.mediaItem)
        #expect(media.fileHandle == nil)
        #expect(media.previewItemURL != nil)
        #expect(media.previewItemURL.map { FileManager.default.fileExists(atPath: $0.path) } == true)
    }

    @Test
    func sentLocalEchoKeepsItsPreviewPage() async throws {
