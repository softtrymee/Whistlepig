...
final class MediaUploadPreviewScreenViewModelTests {
    ...
    func gallery() async throws {
        ...
        try await deferredDismiss.fulfill()
        #expect(timelineProxy.sendGalleryItemInfosCaptionIntentionalMentionsInReplyToEventIDCallsCount == 1)
        #expect(timelineProxy.sendGalleryItemInfosCaptionIntentionalMentionsInReplyToEventIDReceivedArguments?.itemInfos.count == 3)
        #expect(timelineProxy.sendImageUrlThumbnailURLImageInfoCaptionIntentionalMentionsRequestHandleCallsCount == 0)
        #expect(timelineProxy.sendFileUrlFileInfoCaptionIntentionalMentionsRequestHandleCallsCount == 0)
    ...
    func galleryDisabledSendsIndividualMessages() async throws {
        ...
        try await deferredDismiss.fulfill()
        #expect(timelineProxy.sendGalleryItemInfosCaptionIntentionalMentionsInReplyToEventIDCallsCount == 0)
        #expect(timelineProxy.sendFileUrlFileInfoCaptionIntentionalMentionsRequestHandleCallsCount == 2)
        #expect(timelineProxy.sendImageUrlThumbnailURLImageInfoCaptionIntentionalMentionsRequestHandleCallsCount == 1)
    }
    ...
    private func setUpViewModel(urls: [URL],
        ...
        timelineProxy.sendAudioUrlAudioInfoCaptionIntentionalMentionsRequestHandleClosure = { [weak self] _, _, caption, _, _ in
            self?.verifyCaption(caption, expectedCaption: expectedCaption) ?? .failure(.sdkError(TestError.unknown))
        }
        timelineProxy.sendFileUrlFileInfoCaptionIntentionalMentionsRequestHandleClosure = { [weak self] _, _, caption, _, _ in
            self?.verifyCaption(caption, expectedCaption: expectedCaption) ?? .failure(.sdkError(TestError.unknown))
        }
        timelineProxy.sendImageUrlThumbnailURLImageInfoCaptionIntentionalMentionsRequestHandleClosure = { [weak self] _, _, _, caption, _, _ in
            self?.verifyCaption(caption, expectedCaption: expectedCaption) ?? .failure(.sdkError(TestError.unknown))
        }
        timelineProxy.sendVideoUrlThumbnailURLVideoInfoCaptionIntentionalMentionsRequestHandleClosure = { [weak self] _, _, _, caption, _, _ in
            self?.verifyCaption(caption, expectedCaption: expectedCaption) ?? .failure(.sdkError(TestError.unknown))
        }
        timelineProxy.sendGalleryItemInfosCaptionIntentionalMentionsInReplyToEventIDClosure = { [weak self] _, caption, _, _ in
